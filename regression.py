...
class Simple_Regression:
    ...
    def run_model(self, X_tr, y_tr, X_test, y_test):
        ...
        for k in k_all:
            ...
            k/=10
            k_list.append(k)
            reg_v = linear_model.Ridge(alpha=k)
            reg_omega = linear_model.Ridge(alpha=k)
            ###Velocity
            ...
            #IN SAMPLE TEST
            y_pred_train_v = self.prediction(w_b, X_train)
             # ###velocity range
            ...
            ###omega range
            y_pred_train_o = self.prediction(w_b_omega, X_train)
        ...
        ###Velocity
        reg_v = linear_model.Ridge(alpha=final_kv)
        reg_v.fit(self.X_train_og, self.y_train_og[:,0])
        w_bv = reg_v.coef_
        print(w_bv)
        y_pred_test_v = self.prediction(w_bv, self.X_test)
        ...
        ###omega range
        reg_omega = linear_model.Ridge(alpha=final_ko)
        reg_omega.fit(self.X_train_og, self.y_train_og[:,1])
        w_bo = reg_omega.coef_
        y_pred_test_o = self.prediction(w_bo, self.X_test)
    ...
    def learning_curves(self, X,y):
        train_sizes, train_scores, test_scores = learning_curve(linear_model.Ridge(alpha=1), X, y, cv=10, n_jobs=-1, train_sizes=np.linspace(0.01, 1.0, 50))
